...
describe Dessert do
  ...
  describe '#initialize' do
    it 'takes in a type' do
      expect(brownie.type).to eq('brownie') #be checks object_id
    end
    ...
    it 'raises an error if the amount is not a number' do
      expect { Dessert.new("brownie", "tons") }.to raise_error('Amount must be a number') #always pass a raise error matcher
    end
  end
